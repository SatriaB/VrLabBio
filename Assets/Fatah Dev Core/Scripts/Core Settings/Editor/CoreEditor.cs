    ...
    public static class CoreEditor
    {
        ...
        private static void Init()
        {
            ...
            if (coreSettings == null)
            {
                ...
                Debug.LogWarning("[Fatah Dev Core]: Core Settings asset cannot be found in the project. This asset is required for the proper functionality of the modules.");

                coreSettings = ScriptableObject.CreateInstance<CoreSettings>();

                FOLDER_CORE = Path.Combine("Assets", "Fatah Dev Core");

                if (!AssetDatabase.IsValidFolder(FOLDER_CORE))
                {
                    AssetDatabase.CreateFolder("Assets/", "Fatah Dev Core");
                }

                AssetDatabase.CreateAsset(coreSettings, Path.Combine("Assets", "Fatah Dev Core", "Core Settings.asset"));
